...
$Details = foreach ($Row in $RawDetails) {
    ...
    [PSCustomObject]@{
        ...
        "商品名" = $Row.'商品名'
        "异动指标" = $Row.'异动指标'
        "异动来源" = Get-FieldValue $Row @("异动来源", "异动识别方式", "anomaly_source", "detection_mode")
        "当前比较窗口" = Get-FieldValue $Row @("当前比较窗口", "current_window")
        "基准比较窗口" = Get-FieldValue $Row @("基准比较窗口", "baseline_window", "previous_window")
        "当前流量值" = Get-FieldValue $Row @("当前流量值", "current_traffic")
        "基准流量值" = Get-FieldValue $Row @("基准流量值", "baseline_traffic", "previous_traffic")
        "流量变化率" = Get-FieldValue $Row @("流量变化率", "traffic_change_rate")
        "当前转化率" = Get-FieldValue $Row @("当前转化率", "current_conversion_rate")
        "基准转化率" = Get-FieldValue $Row @("基准转化率", "baseline_conversion_rate", "previous_conversion_rate")
        "转化率变化率" = Get-FieldValue $Row @("转化率变化率", "conversion_change_rate")
        "异动识别方式" = Get-FieldValue $Row @("异动识别方式", "异动来源", "detection_mode", "anomaly_source")
        "趋势候选原因" = $Row.'趋势候选原因'
        "趋势判断窗口" = $Row.'趋势判断窗口'
        "历史判断" = $Row.'历史判断'
        "待复核原因" = $Row.'待复核原因'
...
<main class="dashboard">
  ...
  <section class="lower-grid">
    ...
    <div class="panel pink table-panel review-detail-grid">
      ...
      <div class="table-wrap"><table id="reviewDetailTable"><thead><tr><th>父ASIN</th><th class="product">商品名</th><th>异动指标</th><th>当前流量值</th><th>基准流量值</th><th>流量变化率</th><th>当前转化率</th><th>基准转化率</th><th>转化率变化率</th><th>异动来源</th><th>趋势判断窗口</th><th>待复核原因</th><th>历史判断</th></tr></thead><tbody></tbody></table></div>
    </div>
...
function renderDetails() {
  ...
  detailTable.querySelector("tbody").innerHTML = rows.map(row => `<tr>
    ...
    <td>${esc(row["商品名"])}</td><td>${esc(row["异动指标"])}</td><td>${esc(row["异动来源"] || row["异动识别方式"])}</td><td>${esc(row["当前比较窗口"])}</td><td>${esc(row["基准比较窗口"])}</td>
    <td>${esc(row["当前流量值"])}</td><td>${esc(row["基准流量值"])}</td><td>${esc(row["流量变化率"])}</td><td>${esc(row["当前转化率"])}</td><td>${esc(row["基准转化率"])}</td><td>${esc(row["转化率变化率"])}</td><td>${esc(row["趋势候选原因"])}</td><td>${esc(row["趋势判断窗口"])}</td><td>${esc(row["历史判断"])}</td><td>${esc(reviewReason(row))}</td><td class="${row["最终状态"] === "高优先级异动" ? "status-high" : "status-review"}">${esc(row["最终状态"])}</td><td>${esc(row["高优先级类型"])}</td>
  </tr>`).join("");
}
function renderReviewDetails() {
  ...
  reviewDetailTable.querySelector("tbody").innerHTML = rows.map(row => `<tr>
    ...
    <td>${esc(row["当前流量值"])}</td><td>${esc(row["基准流量值"])}</td><td>${esc(row["流量变化率"])}</td><td>${esc(row["当前转化率"])}</td><td>${esc(row["基准转化率"])}</td><td>${esc(row["转化率变化率"])}</td><td>${esc(row["异动来源"] || row["异动识别方式"])}</td><td>${esc(row["趋势判断窗口"])}</td><td>${esc(reviewReason(row))}</td><td>${esc(row["历史判断"])}</td>
  </tr>`).join("");
}
